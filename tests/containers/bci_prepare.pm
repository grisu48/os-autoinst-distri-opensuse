...
sub packages_to_install {
    ...
    # On SLES we need to enable the SDK module for python3-pip. The SDK module requires the Desktop module
    # On SLES12 we also need PackageHub for jq.
    ...
    if ($host_distri eq 'ubuntu') {
        ...
        push @packages, ('python3-pip');
        push @packages, ('python3-virtualenv') if ($bci_virtualenv);
    } elsif ($host_distri =~ /sles|leap/) {
        my $version = "$version.$sp";
        push @packages, ('python3-virtualenv') if ($bci_virtualenv);
        push @packages, ('skopeo');
        if ($version =~ /12\./) {
            die "virtualenv is not supported on SLES12" if ($bci_virtualenv);
        ...
        push @packages, qw(skopeo python3-pip);
        push @packages, ('python3-virtualenv') if ($bci_virtualenv);
    } else {
